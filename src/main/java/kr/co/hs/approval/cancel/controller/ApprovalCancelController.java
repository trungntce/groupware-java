package kr.co.hs.approval.cancel.controller;

import kr.co.hs.approval.cancel.service.ApprovalCancelService;
...
public class ApprovalCancelController {
    ...
    @Autowired
    private ApprovalCancelService approvalCancelService;
    ...
    public String goList(HttpServletRequest request, Model model, ApprovalSearchDTO searchDTO){
        searchDTO = approvalCancelService.initParam(request, searchDTO);
    ...
    public String goDetail(Model model, ApprovalSearchDTO searchDTO){
        ...
        return "/approve/cancel/detail";
    }

    @TilesDynamic("base")
    @GetMapping("/re-register.hs")
    public String goReRegister(Model model, HttpServletRequest request, ApprovalSearchDTO searchDTO){
        ApprovalSignLineSearchDTO signSearch = new ApprovalSignLineSearchDTO();
        signSearch.setApprovalId(searchDTO.getApprovalId());
        model.addAttribute("form", approvalService.selectOne(searchDTO));
        model.addAttribute("approvalRole", codeService.getCodeList("approval_role", LocaleContextHolder.getLocale().toString()));
        model.addAttribute("empSign", approvalSignLineService.selectList(signSearch));
        model.addAttribute("approvalFiles", approvalFileService.selectFileByApprovalId(new ApprovalFileSearchDTO(searchDTO.getApprovalId())));
        return "/approve/cancel/re-register";
    }

    @PostMapping("/re-register.hs")
    public String doReRegister(HttpServletRequest request, @ModelAttribute("form") ApprovalDTO approvalDTO){
        approvalCancelService.register(request, approvalDTO);
