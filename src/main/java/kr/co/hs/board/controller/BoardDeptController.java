...
import kr.co.hs.board.service.BoardDeptService;
...
public class BoardDeptController {
    final BoardDeptService datBoardService;
    ...
    @Autowired
    private BoardCompanyService boardCompanyService_;
    ...
    @TilesDynamic("base")
    @GetMapping("/board/notice/department/list.hs")
    public String list(HttpServletRequest request, Model model, @ModelAttribute("search") BoardControlDTO boardControlDTO) {
        ...
        model.addAttribute("lstDeptLevel",deptService.selectDeptBoard(langCd));
        ...
        return "/hrga/deptnotice/list";
    }

    @RequestMapping(value = "/springPaginationDataTablesBoardDept", method = RequestMethod.GET, produces = "application/json")
    public @ResponseBody String springPaginationDataTablesBoardCompany(HttpServletRequest request) throws IOException {
        ...
        boardControlDTO.setEmpCd(emp.getEmpCd());
        boardControlDTO.setLangCd(langCd);
        if (emp.getDeptCd().equals(2) || emp.getDeptCd().equals(1) ){
            sum_row = datBoardService.countAdmin(boardControlDTO);
        }
        else {
            if (emp.getTranslationYn().equals("Y")||emp.getAdminYn().equals("Y")){
                sum_row = datBoardService.countTranslation(boardControlDTO);
            }
            else {
        ...
        if (emp.getDeptCd().equals(2) || emp.getDeptCd().equals(1) || emp.getTranslationYn().equals("Y")||emp.getAdminYn().equals("Y")){
            sum_row = datBoardService.countAdmin(boardControlDTO);
        }
        else {
            sum_row = datBoardService.count(boardControlDTO);
...
//            }
    ...
    private List<BoardControlDTO> createPaginationDataBoard(Integer pageNumber, Integer pageDisplayLength, String searchParameter, HttpServletRequest request, String dateSql, String deptSearch, String inputSearchSql, String translationStatus, String str) {
        ...
        if (emp.getDeptCd().equals(2) || emp.getDeptCd().equals(1) || emp.getTranslationYn().equals("Y")||emp.getAdminYn().equals("Y") || emp.getTranslationYn().equals("Y")||emp.getAdminYn().equals("Y")){
            ttList = datBoardService.getCompanyNoticeListAdmin(boardControlDTO);
        }
        else {
            ttList = datBoardService.getCompanyNoticeList(boardControlDTO);
//            if (emp.getTranslationYn().equals("Y")||emp.getAdminYn().equals("Y")){
//                ttList = datBoardService.getCompanyNoticeListTranslation(boardControlDTO);
//            }
//            else {
//                ttList = datBoardService.getCompanyNoticeList(boardControlDTO);
//            }
    ...
    //show write page
    @TilesDynamic("base")
    @GetMapping("/board/notice/department/write.hs")
    public String goWrite(Model model, @ModelAttribute("board") BoardControlDTO boardControlDTO, HttpServletRequest request) {
        ...
        model.addAttribute("selectDepartmentList", boardCompanyService_.selectDepartmentList(param));
        ...
        log.error("emp/write @@@@@@@@@");
        return "/hrga/deptnotice/write";
    }

    //method insert for board
    @PostMapping("/board/notice/department/write.hs")
    public String doWrite(Model model, @ModelAttribute("board") BoardControlDTO boardControlDTO) {
        ...
        return "redirect:/board/notice/department/list.hs";
    }

    @TilesDynamic("base")
    @GetMapping("/board/notice/department/view.hs")
    public String view(Model model, @ModelAttribute("search") BoardControlDTO boardControlDTO, HttpServletRequest request) {
        ...
        return "/hrga/deptnotice/view";
    }

    @TilesDynamic("base")
    @GetMapping("/board/notice/department/edit.hs")
    public String goEdit(Model model, @ModelAttribute("board") BoardControlDTO boardControlDTO) {
        ...
        log.error("emp/write @@@@@@@@@");
        return "/hrga/deptnotice/edit";
    }

    @PostMapping("/board/notice/department/edit.hs")
    public String doEdit(Model model, @ModelAttribute("board") BoardControlDTO boardControlDTO) {
        ...
        return "redirect:/board/notice/department/view.hs?boardId="+boardId+paraBackSearch;
    }
}
