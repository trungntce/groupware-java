                    ...
                    <form id="myform" class="form-inline" action="">
                        <table class="table mb-0 w-100">
                            ...
                            <tr>
                                ...
                                <td><input id="title" name="contents" class="form-control form-control-sm mr-1 w-100" type="text" value="${workDTO.title}"></td>
                            </tr>
                                        ...
                                        <c:forEach items="${workChild}" var="lst">
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <tr id="row-${lst.id}">
                                                        <td><input id="${lst.id}" name="contents" class="form-control form-control-sm mr-1 w-100" type="text" value="${lst.contents}"></td>
                                                        <td><button class="btn btn-success" onclick="removeRowContent('row-${lst.id}','${lst.id}')"><i class="fas fa-minus"></i></button></td>
                                                    </tr>
                                                </c:otherwise>
                                            </c:choose>
                                        </c:forEach>
...
<script>
    ...
    function EditW() {
        ...
        if (isValid == true) {
            ...
            $.ajax(settings).done(function (response) {
                location.href = "/work/mydaywork/detail.hs?id=${workDTO.dayWorkId}&title=success&message=Edit sucess&<%=backSearch%>";
                off();
            });
        }
    }
    ...
    function back() {
        location.href = "/work/mydaywork/detail.hs?id=${workDTO.dayWorkId}&<%=backSearch%>";
    }
    ...
    function goEvent() {
        ...
        if (isValid == true) {
            ...
            $.ajax(settings).done(function (response) {
                location.href = "/work/mydaywork/detail.hs?id=${workDTO.dayWorkId}&title=success&message=Add new sucess&<%=backSearch%>";
            });
        } else {
            off();
        }
