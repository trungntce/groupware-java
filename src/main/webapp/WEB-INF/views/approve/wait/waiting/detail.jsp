    ...
    <div class="row">
        ...
        <div class="col-xl-4 ui-sortable">
                                ...
                                <c:forEach items="${empSign}" var="lst" varStatus="loop">                                       
                                    <tr>
                                        ...
                                        <c:if test="${lst.empCd eq _userInfo.empCd 
                                            && lst.approvalStatus == 1
                                            && form.translationId != null
                                            && (loop.index == 0 || (loop.index != 0 && empSign.get(loop.index-1).approvalStatus == 3))}">
                                            <c:set var="isSign" value="1"/>
                                            <c:set var="idSignLine" value="${lst.signLineId}" />
                                        </c:if>
                                        <c:if test="${lst.empCd eq _userInfo.empCd && lst.signatureImagePath == ''}">
                                            <c:set var="isSign" value="2"/>
                                        </c:if>
                                    </tr>
                                </c:forEach>
                            </c:if>
            ...
            <div class="panel panel-inverse mt-2">
                <div class="panel-body">
                    <div class="row">
                        <form id="frm-confirm-approval" method="post" action="${_ctx}/approval/wait/waiting/update.hs">
...
<c:if test="${isSign == '2'}">
    <script>
        if(confirm("<spring:message code='common.message.setup.signature.title'/>")){
            location.href = "${_ctx}/approval/setup/signature/index.hs"
        }else{
            location.href = "${_ctx}/approval/wait/waiting/list.hs"
        }
    </script>
</c:if>
<script>
