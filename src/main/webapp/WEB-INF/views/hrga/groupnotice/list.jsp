...
<div id="content" class="content">
	<h1 class="page-header cursor-default">
		<spring:message code='dateboard.title' /> - <spring:message code='dateboard.notice.company.nhom' />
		<br>
		<small><spring:message code='dateboard.notice.group.detail' /></small>
	</h1>

	<div class="panel panel-inverse">
		<div class="panel-heading"><h4 class="panel-title"><spring:message code='dateboard.notice.company.timkiem' /></h4></div>
		<div class="row">
			...
			<script type="text/javascript">
				...
				$(document).ready(function () {
					...
					var table = $('#tbwork').DataTable({
						...
						"sAjaxSource": "/springPaginationDataTablesBoardGroup?start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val()+"&translationStatus=" + $('#translationStatus').val(),
						"columns": [
							{
								"data": "boardId",
					...
					$('#tbwork').on( 'page.dt', function () {
						...
						location.href = "/board/notice/group/list.hs?start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val()+"&translationStatus=" + $('#translationStatus').val() + "&displayStartPra=" + displayStartPra;
					} );

				});
				function addW() {
					location.href = "/board/notice/group/write.hs";
				};
					...
					if (lstChecked.trim() == "") {
						...
						$.ajax({
							...
							success: function (response) {
								location.href = "/board/notice/group/list.hs?title=success&message=Delete sucess";
							},
				...
				function detail(id) {
					var displayStartPra = $('#tbwork').DataTable().page.info().page * 10;
					location.href = "/board/notice/group/view.hs?boardId=" + id+ "&start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val()+"&translationStatus=" + $('#translationStatus').val() + "&displayStartPra=" + displayStartPra;
				}
			</script>