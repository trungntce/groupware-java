...
<div id="content" class="content">
	<h1 class="page-header cursor-default">
		<spring:message code='dateboard.title' /> - <spring:message code='dateboard.notice.company.cty' />
		<br>
		<small><spring:message code='dateboard.notice.company.detail' /></small>
	</h1>
	...
	<div>
		<c:choose>
            <c:when test="${adminYn == 'Y'}">
				<button class="open-AddBookDialog btn btn-info mb-1" type="button" onclick="openModel()"><spring:message code='work.list.delete' /></button>
			</c:when>
		</c:choose>
		<button class="btn btn-primary mb-1" onclick="addW()"><spring:message code='dateboard.notice.company.add' /></button>
	</div>
			...
			<script type="text/javascript">
				...
				$(document).ready(function () {
					...
					var table = $('#tbwork').DataTable({
						...
						"sAjaxSource": "/springPaginationDataTablesBoardCompany?start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val()+"&translationStatus=" + $('#translationStatus').val(),
						"columns": [
							{
								"data": "boardId",
					...
					$('#tbwork').on( 'page.dt', function () {
						...
						location.href = "/board/notice/company/list.hs?start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val() +"&translationStatus=" + $('#translationStatus').val()+ "&displayStartPra=" + displayStartPra;
					} );
				});
				function addW() {
					location.href = "/board/notice/company/write.hs";
				};
				function searchMethod() {
					// $('#tbwork').DataTable().ajax.url("/springPaginationDataTablesBoardCompany?start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val()+"&translationStatus=" + $('#translationStatus').val()).load();
					...
					if (lstChecked.trim() == "") {
						...
						$.ajax({
							...
							success: function (response) {
								location.href = "/board/notice/company/list.hs?title=success&message=Delete sucess";
							},
				...
				function detail(id) {
					var displayStartPra = $('#tbwork').DataTable().page.info().page * 10;
					location.href = "/board/notice/company/view.hs?boardId=" + id+ "&start_date=" + $('#start_date').val() + "&end_date=" + $('#end_date').val() + "&levelDept=" + $("#levelDept").val() + "&optionSearch=" + $('#optionSearch').val() + "&inputSearch=" + $('#inputSearch').val() +"&translationStatus=" + $('#translationStatus').val()+ "&displayStartPra=" + displayStartPra;
				}
