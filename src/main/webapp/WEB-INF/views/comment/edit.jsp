...
%>
                                ...
                                <table class="table mb-0">
                                    <tbody>
                                            ...
                                            <td style="width: 35%">
                                                <select style="width: 40%;" id="language" name="language"
                                                    ...
                                                    <c:forEach items="${lstLang}" var="lstL">
                                                        <option ${lstL.str} value="${lstL.langCd}">${lstL.langName}
                                                        </option>
                                                    </c:forEach>
                                                </select>
                                            </td>
                                        ...
                                        <tr>
                                            ...
                                            <td style="width: 35%"><textarea id="contents" name="contents"
                                                    class="ckeditor form-control" style="width:100%" rows="2"
                                                    cols="80">${commentDTO.contents}</textarea></td>
                                        </tr>
                                        <tr>
                                            <td style="width: 15%">
        ...
        <script>
            ...
            function add() {
                ...
                if (isValid == true) {
                    var settings = {
                        "url": "/translational/doeditcomment.hs",
                        "method": "POST",
                        "headers": {
                        ...
                        "data": JSON.stringify({
                            ...
                            "dayWorkCommentId": '${commentDTO.dayWorkCommentId}',
                            "dayWorkId": '${dayWorkId}',
                            "type": '${loai}'
                        }),
                    };
                    $.ajax(settings).done(function (response) {
                        if (response == "daywork") {
                            location.href = "/work/daywork/detail.hs?id=${dayWorkId}&title=success&message=Add new sucess&<%=backSearch%>";
                        } else {
                            location.href = "/work/mydaywork/detail.hs?id=${dayWorkId}&title=success&message=Add new sucess&<%=backSearch%>";
                        }
                    });
                }
            }
